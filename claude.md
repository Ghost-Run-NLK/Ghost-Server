...
- **유저당 ACTIVE 1개** 정책 — start 시 기존 ACTIVE 를 ABANDONED 로 자동 폐기 (`findByUserIdAndStatus` + `RunSession.abandon`) + 30초 idle 시 `RunSessionAbandonScheduler` 가 ABANDONED. **ABANDONED 전이 시 해당 런의 TrackPoint 는 일괄 삭제** (메타데이터만 남김)
